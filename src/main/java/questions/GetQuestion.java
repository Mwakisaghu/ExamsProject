...
public class GetQuestion implements HttpHandler {
    ...
    public void handleRequest(HttpServerExchange exchange) throws Exception {
        // Extracting the "questionId" path variable using RestUtils
        String strQuestionId = RestUtils.getPathVar(exchange, "questionId");

        try {
            // Converting the "questionId" to an integer
            assert strQuestionId != null;
            int questionId = Integer.parseInt(strQuestionId);

            // SQL query to retrieve question data by questionId
            String selectQuery = "SELECT * FROM questions WHERE question_id = ?";
            ...
            List<LinkedHashMap<String, Object>> questionMap = QueryManager.executeSelectQuery(selectQuery, (HashMap<Integer, Object>) paramMap);

            if (!questionMap.isEmpty()) {
                // The query result is a list of rows, and each row is a LinkedHashMap
                LinkedHashMap<String, Object> questionData = questionMap.get(0);

                // Creating a JSON response
                Gson gson = new GsonBuilder().registerTypeAdapter(LocalDateTime.class, new LocalDateTimeAdapter()).create();
                String strJsonResponse = gson.toJson(questionData);
                ...
                // Guardian not found
                StatusResponses.send404NotFoundResponse(exchange, "Error: Question not found");
            }
        } catch (NumberFormatException e) {
            // Handle the case where an invalid questionId is provided - bad req
            StatusResponses.sendErrorResponse(exchange, "Error: Invalid questionId");
        } catch (Exception e) {
            // Handle other errors
            e.printStackTrace();
