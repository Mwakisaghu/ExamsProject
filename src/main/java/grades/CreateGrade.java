...
public class CreateGrade implements HttpHandler {
    public void handleRequest(HttpServerExchange exchange) throws ParserConfigurationException, IOException, NoSuchAlgorithmException, SAXException {
        ...
        try {
            ...
            StatusResponses.sendSuccessResponse(exchange, "Grade Created successfully");
        } finally {
            if (connection != null) {
                try {
                    connection.close();
    ...
    private boolean isValidRequest(Map<String, Object> requestBody) {
        ...
        return requestBody.containsKey("minimum_score") && requestBody.containsKey("maximum_score")
                && requestBody.containsKey("grade");
    }
}
