...
public class CreateExam implements HttpHandler {
    public void handleRequest(HttpServerExchange exchange) throws ParserConfigurationException, IOException, NoSuchAlgorithmException, SAXException {
        ...
        try {
            ...
            StatusResponses.sendSuccessResponse(exchange, "Exam Created successfully");
        } finally {
            if (connection != null) {
                try {
                    connection.close();
    ...
    private boolean isValidRequest(Map<String, Object> requestBody) {
        ...
        return requestBody.containsKey("serial_number") && requestBody.containsKey("exam_name") &&
                requestBody.containsKey("exam_period");
    }
}
